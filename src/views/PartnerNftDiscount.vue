<template>
  <div class="container text-center">
    <div class="text-align-header mt-4 mb-2">
      <h1>Get a {{ getMinterDiscountPercentage }}% discount on minting a {{getTldName}} name!</h1>
      <p>
        If you hold one of the supported partner NFTs.
      </p>
    </div>
    ...
    <div class="text-align-header">
      <p class="mt-5 price-text">
          Domain price: {{Number(getPrice).toFixed(5)}} {{getPaymentTokenName}}
      </p>
    </div>
      ...
      <div class="col-md-6 offset-md-3 table-container">
        <table class="table table-borderless table-ppl">
          ...
          <tbody>
            ...
            <tr>
              <td>6+ characters</td>
              <td>{{this.getFormattedPrice(getMinterTldPrice6)}} {{getPaymentTokenName}}</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    getPrice() {
      ...
      if (this.chosenDomainName) {
        if (this.chosenDomainName.match(/./gu).length === 1) {
          chosenPrice = this.getMinterTldPrice1;
        } else if (this.chosenDomainName.match(/./gu).length === 2) {
          chosenPrice = this.getMinterTldPrice2;
        } else if (this.chosenDomainName.match(/./gu).length === 3) {
          chosenPrice = this.getMinterTldPrice3;
        } else if (this.chosenDomainName.match(/./gu).length === 4) {
          chosenPrice = this.getMinterTldPrice4;
        } else if (this.chosenDomainName.match(/./gu).length === 5) {
          chosenPrice = this.getMinterTldPrice5;
        }
      }

      if (!chosenPrice) {
        chosenPrice = this.getMinterTldPrice6;
      }
      
      // deduce discount from price
      chosenPrice = Number(chosenPrice) * (1 - (Number(this.getMinterDiscountPercentage) / 100));
  ...
  methods: {
    ...
    async buyDomain() {
      ...
      try {
        ...
        const tx = await minterContractSigner.partnerFreeMint(
    ...
    getFormattedPrice(price) {
      // deduce discount from price
      price = Number(price) * (1 - (Number(this.getMinterDiscountPercentage) / 100));
